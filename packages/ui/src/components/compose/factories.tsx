...
import { ColorVariantKey, UIComponentProps, UIStyleCategory } from '@trms/theme'
...
export type WrapUIProps<
  ...
  P = {},
> = AsComponentProps<As, UIProps<P>>

const throwIfInvalidTag = (as: React.ElementType, validTags: React.ElementType[] = []) => {
  if (validTags?.length && !validTags.includes(as)) {
    throw new TypeError(`${as} is not a valid HTML element to create this component type.`)
  }
}

type AsTags<As extends React.ElementType = React.ElementType> = As[]

type ComponentFactoryConfig<As extends React.ElementType, P = {}, Props = UIProps<P>> = {
  as?: As
  displayName: string
...
>(
  ...
  const UiComponent = forwardRefAs((props, ref) => {
    ...
    const [uiProps, nonUIProps] = useTW(rest, [...excludeProps, ...omittableStyleCategories])
