...
export const toUnit = (value: DimensionType | undefined, unit = 'px'): string => {
  if (value == null) {
    return '0'
  } 
...
const resolveThemeColor = (colors: Theme['colors'], prop: keyof CSSProperties) => (value: string): CSSObject => {
 ...
 * @internal
 */
export function _responsiveColor<T>(
  theme: Theme,
  values: T[],
  callback: (value: T, index?: number, array?: T[]) => CSSObject
): CSSObject[] {
...
export const useResponsiveSpace = (
  ...
  return _responsive(theme.breakpoints, spaceKeys, (spaceKey) =>
    _fillCSSObject(props, toRem(theme.spacing[spaceKey]))
  );
};
...
function useColors(
  ...
  const bgResolver = resolveThemeColor(colors, 'backgroundColor')
  const fgResolver = resolveThemeColor(colors, 'color')

  if (background) {
    backgroundColor = _responsive<string>(breakpoints, [background], bgResolver)[0]
  }
  if (foreground) {
    color = _responsive<string>(breakpoints, [foreground], fgResolver)[0]
  }
