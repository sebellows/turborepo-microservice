...
function makeColor<T extends ColorParam = ColorParam>(
  obj?: T,
  model?: ColorModel
) {
  return new ColorService(obj, model);
}
export type ColorServiceInstance<T extends ColorParam> = ReturnType<
  typeof makeColor<T>
>;
...
export class ColorService<T extends ColorParam = ColorParam> {
  ...
  get model() {
    const colorModel = (
      this.color as ColorProviderInstance<T> & { model: ColorModel }
    ).model;
  ...
  saturationl<V extends number | undefined>(
    ...
    return val
      ? makeColor(this.color.saturationl(val))
      : this.color.saturationl();
  }
  ...
  saturationv<V extends number | undefined>(
    ...
    return val
      ? makeColor(this.color.saturationv(val))
      : this.color.saturationv();
  }
  ...
  mix(
    mixinColor: ColorProviderInstance,
    weight?: number
  ): ColorProviderInstance {
