...
import { AsyncClosure, AsyncFunction, Closure, Constructor, Type, ValueOf } from './types'
...
export function isClosure<T = any>(value: unknown, strict?: boolean): value is Closure<T> {
  if (strict) {
...
export function isCallable<T>(value: unknown): value is Closure<T> | AsyncClosure<T> {
